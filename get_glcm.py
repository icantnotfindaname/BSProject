...
def calcu_glcm(img, vmin=0, vmax=255, nbit=64, slide_window=5, step=[2], angle=[0]):
    ...
    img2 = cv2.copyMakeBorder(img1, floor(slide_window/2), floor(slide_window/2), floor(
        slide_window/2), floor(slide_window/2), cv2.BORDER_REPLICATE)  # 图像扩充
    ...
    for i in range(patch.shape[2]):
        for j in range(patch.shape[3]):
            glcm[:, :, :, :, i, j] = greycomatrix(
                patch[:, :, i, j], step, angle, levels=nbit)
...
def calcu_glcm_Auto_correlation(glcm, nbit=64):
    ...
    '''
    Auto_correlation = np.zeros(
        (glcm.shape[2], glcm.shape[3]), dtype=np.float32)
    for i in range(nbit):
        for j in range(nbit):
